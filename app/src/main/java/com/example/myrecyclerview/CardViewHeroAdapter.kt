...
class CardViewHeroAdapter(private val listHero: ArrayList<Hero>) :
  ...
  override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): CardViewViewHolder {
    val view: View = LayoutInflater.from(parent.context).inflate(R.layout.item_cardview_hero,
      parent, false)
  ...
  override fun onBindViewHolder(holder: CardViewViewHolder, position: Int) {
    ...
    holder.btnShare.setOnClickListener { Toast.makeText(holder.itemView.context, "Share " +
            listHero[holder.adapterPosition].name, Toast.LENGTH_SHORT).show() }
    holder.itemView.setOnClickListener { Toast.makeText(holder.itemView.context, "You choose " +
            listHero[holder.adapterPosition].name, Toast.LENGTH_SHORT).show() }
  }
